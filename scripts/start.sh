...
if command -v pm2 >/dev/null 2>&1; then
  ...
  setsid -f node server.js >/tmp/codex-gateway-pool/server.log 2>&1
  sleep 1
  NEW_PID="$(pgrep -n -f 'node server.js' || true)"
  if [[ -n "$NEW_PID" ]]; then
    echo "$NEW_PID" >/tmp/codex-gateway-pool/pid
    echo "Started PID $NEW_PID"
  else
    echo "Started, but no PID was captured."
  fi
fi
