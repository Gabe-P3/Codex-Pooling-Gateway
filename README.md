...
- OpenAI-style endpoint support:
  ...
  - `/api/openai/v1/modes`
  - `/api/openai/models`
- Dynamic model list from Codex `models_cache.json` when available
- More robust Codex output extraction from outfile, JSONL stdout, and stderr transcript fallbacks
...
- starts service (PM2 if available, otherwise native platform scripts)
...
- `codexDefaultCwd`: default working directory for execute requests (defaults to project root)
- `codexTempDir`: temp/output dir for Codex exec runs (defaults to OS temp dir)
- `codexModelsCacheFile`: optional override for `models_cache.json`
- `codexBin`: optional override for the `codex` executable/script
...
### Windows execute requests fail with `/root` or missing path errors

Restart the server after updating so it picks up the Windows-safe default working directory. Use `powershell -ExecutionPolicy Bypass -File .\scripts\stop.ps1` then `powershell -ExecutionPolicy Bypass -File .\scripts\start.ps1`.
